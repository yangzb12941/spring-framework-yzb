...
public abstract class AbstractSingleBeanDefinitionParser extends AbstractBeanDefinitionParser {
	 ...
	 *
	 * 在 parseInternal 中并不是直接调用自定义的 doParse 函数，而是进行了一系列的数据准备包括对 beanClass、scope、lazyInit 等属性的准备。
	 *
	 * 回顾一下全部的自定义标签处理过程，虽然在实例中我们定义 UserBeanDefinitionParser，
	 * 但是在其中我们只是做了与自己业务逻辑相关的部分。不过我们没做但是并不代表没有，在这个处理过程中同样也是按照Spring中默认标签的处理方式进行，
	 * 包括创建 BeanDefinition 以及进行相应默认属性的设置，对于这些工作 Spring 都默默地帮我们实现了，
	 * 只是暴露出一些接口来供用户实现个性化的业务。
	 *
	 * 到此为止我们已经完成了Spring 中全部的解析工作，也就是说到现在为止我们已经理解了 Spring 将 bean
	...
	protected final AbstractBeanDefinition parseInternal(Element element, ParserContext parserContext) {
		...
		//获取自定义标签中class，此时会调用自定义解析器如 UserBeanDefinitionParser中的 getBeanClass 方法

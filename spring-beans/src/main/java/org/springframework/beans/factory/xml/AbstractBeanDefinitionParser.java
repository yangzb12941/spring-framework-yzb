...
public abstract class AbstractBeanDefinitionParser implements BeanDefinitionParser {
	...
	public final BeanDefinition parse(Element element, ParserContext parserContext) {
		// 虽说是对自定义配置文件的解析，但是，我们可以看到，在这个函数中大部分的代码是用来处理将解析后的 AbstractBeanDefinition 转化为 BeanDefinitionHolder
		// 并注册的功能，而真正去做解析的事情委托给了函数 parseInternal，正是这句代码调用了我们自定义的解析函数。
		// 在 parseInternal 中并不是直接调用自定义的 doParse 函数，
		// 而是进行了一系列的数据准备包括对 beanClass、scope、lazyInit 等属性的准备。
